...
ROOT_DIR="/home/airflow/"
PACKAGE_PATH = os.path.join(ROOT_DIR,"dags", "dist")
...
if not shutil.which("virtualenv"):
    ...
    def finance_complaint_etl():
        ...
        @task.virtualenv(
            ...
            system_site_packages=False,
            requirements=[package_names])
        def deinitialization(config):
            from etl_project.finance_complaint_config.config import PipelineConfig
            from etl_project.finance_complaint_constant import constant
            pipeline_config = config[constant.CONFIG_KEY_NAME][constant.PIPELINE_CONFIG_KEY_NAME]
            pipeline_config = PipelineConfig(*(pipeline_config))
            import shutil
        ...
        pipeline_config = load(config=transform_config_n_output)
        deinitialization(config=pipeline_config)
